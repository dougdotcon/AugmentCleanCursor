...
main() {
    ...
    echo
    read -p "Would you like to clean Cursor databases now? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Prompt user to clean extensions
    echo
    read -p "Would you like to clean Cursor extensions now? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        log_info "Running extensions cleaning..."
        "$AUGMENT_CMD" clean-extensions
    fi

    # Prompt user to modify telemetry IDs
    echo
    read -p "Would you like to modify Cursor telemetry IDs now? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    log_info "  $AUGMENT_CMD clean             - Clean Cursor databases"
    log_info "  $AUGMENT_CMD clean-extensions  - Clean Cursor extensions"
