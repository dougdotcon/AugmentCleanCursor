"""
Cursor telemetry ID modifier module
"""
...
from .utils import (
    ...
    get_cursor_paths, backup_file,
    generate_machine_id, generate_device_id
)

def modify_telemetry_ids() -> bool:
    """
    Modify telemetry IDs in Cursor storage.json file

    Returns:
        True if successful, False otherwise
    """
    info("Starting Cursor telemetry ID modification")
    ...
    if not storage_json.exists():
        warning(f"Cursor storage.json not found at: {storage_json}")
        return False
    ...
    try:
        ...
        info("You may need to restart Cursor for changes to take effect")
